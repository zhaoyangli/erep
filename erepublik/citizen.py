...
class Citizen(CitizenAPI):
    ...
    def login(self):
        ...
        #     self.telegram.send_message(f"*Started.95* {now():%F %T}")
    ...
    def update_job_info(self):
        ...
        print(self._get_main_job_data().json())
        # {'isEmployee': True, 'employer': {'id': 2606437, 'name': 'Kaitou.Kid',
        #                                   'img': 'https://cdnt.erepublik.net/BH40UNU_7yxXRuj3PiD8deDzBHc=/142x142/smart/avatars/Citizens/2010/01/21/deb28be77f49c3d85d0318ad09c34f64.jpg?951da0e68705bec59afa8d1dbc004af4'},
        #  'salary': 1970, 'tax': 19.7, 'currency': 'CNY', 'alreadyWorked': True, 'playerLevel': 237,
        #  'overTime': {'points': 969, 'nextOverTime': 1589011495, 'salary': 3940, 'usableEnergy': 1246}}

        print(ot)
        # {'points': 969, 'nextOverTime': 1589011495, 'salary': 3940, 'usableEnergy': 1246}
        if ot:
            self.my_companies.next_ot_time = localize_timestamp(int(ot.get("nextOverTime", 0)))
            self.ot_points = ot.get("points", 0)

            print(self.my_companies.next_ot_time)
            print(self.ot_points)
            # 2020 - 05 - 09  01:04:55 - 07: 00
            # 969
    ...
    def get_battle_top_10(self, battle_id: int) -> Dict[int, List[Tuple[int, int]]]:
        ...
        resp = self._post_military_battle_console(battle_id, 'battleStatistics', round_id=round_id, division=4).json()
    ...
    def cus_sorted_battles_all_air(self, sort_by_time: bool = True) -> List[int]:
        cs_battles_air: List[int] = []
        cs_battles_ground: List[int] = []
        deployed_battles_air: List[int] = []
        deployed_battles_ground: List[int] = []
        ...
        ret_battles += (cs_battles_air  +
                        deployed_battles_air  +
                        ally_battles_air  +
                        other_battles_air )
        return ret_battles

    def cus_sorted_battles_all_groud(self, sort_by_time: bool = True) -> List[int]:
        cs_battles_air: List[int] = []
        cs_battles_ground: List[int] = []
        deployed_battles_air: List[int] = []
        deployed_battles_ground: List[int] = []
        ...
        ret_battles += ( cs_battles_ground +
                         deployed_battles_ground +
                         ally_battles_ground +
                         other_battles_ground)
        return ret_battles

    def cus_sorted_battles(self, sort_by_time: bool = True) -> List[int]:
        cs_battles_air: List[int] = []
        cs_battles_ground: List[int] = []
        deployed_battles_air: List[int] = []
        deployed_battles_ground: List[int] = []
        ...
        ret_battles += (cs_battles_air + cs_battles_ground +
                        ...
                        ally_battles_air + ally_battles_ground +
                        other_battles_air + other_battles_ground)
        return cs_battles_ground

    def get_empty_status(self, battle_id: int, div: int = 4) -> int:
        # return {}
        low = 10000
        if div == 1:
            low = 2000000
        ...
        resp = self._post_military_battle_console(battle_id, 'battleStatistics', round_id=round_id, division=division).json()
        resp.pop('rounds', None)
        print(resp)
        ...
        for country_id, data in resp.items():
            ...
            if data == []:
                return 1
            if data.get("fighterData") == []:
                print("found empty: ", battle_id, "div: ", div)
                return 1
            else:
                for place in sorted(data.get("fighterData", {}).values(), key=lambda _: -_['raw_value']):
                    ret.append(place['raw_value'])
            if int(ret[0]) < low:
                return 1
        ...
        #     return place
        # for fighter in ret:
        #     if fighter[0] == self.name:
        #         place = ret.index(fighter) + 1
        return result


    def get_my_status(self, battle_id: int, div: int = 4) -> int:
        # return {}
        battle = self.all_battles.get(battle_id)
        round_id = battle.zone_id
        division = div if round_id % 4 >0 else 11
